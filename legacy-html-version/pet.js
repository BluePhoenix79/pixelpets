...
const COSTS = { feed: 10, play: 5, clean: 8, rest: 0, vet: 50, toy: 25 };

const ACHIEVEMENTS = [
    { id: 'perfect_health', name: 'Perfect Health', description: 'Maintain 100% health for your pet', icon: '💚', check: (pet) => pet.health === 100, progress: (pet) => (pet.health / 100) * 100 },
    { id: 'happy_pet', name: 'Happiness Master', description: 'Keep your pet\'s happiness above 90%', icon: '😊', check: (pet) => pet.happiness >= 90, progress: (pet) => (pet.happiness / 90) * 100 },
    { id: 'well_fed', name: 'Gourmet Chef', description: 'Keep your pet well-fed with hunger above 80%', icon: '🍖', check: (pet) => pet.hunger >= 80, progress: (pet) => (pet.hunger / 80) * 100 },
    { id: 'clean_pet', name: 'Squeaky Clean', description: 'Maintain cleanliness above 85%', icon: '✨', check: (pet) => pet.cleanliness >= 85, progress: (pet) => (pet.cleanliness / 85) * 100 },
    { id: 'energetic', name: 'Full of Energy', description: 'Keep energy levels above 75%', icon: '⚡', check: (pet) => pet.energy >= 75, progress: (pet) => (pet.energy / 75) * 100 },
    { id: 'wealthy_balance', name: 'Wealthy Saver', description: 'Reach a balance of $100 or more', icon: '💰', asyncCheck: async (userId) => { const { data } = await supabase.from('user_finances').select('balance').eq('user_id', userId).maybeSingle(); return data && data.balance >= 100; }, progress: (pet, stats = {}) => Math.min(100, ((stats.balance || 0) / 100) * 100) },
    { id: 'big_spender', name: 'Big Spender', description: 'Spend $100 total on your pet', icon: '🧾', asyncCheck: async (userId) => { const { data } = await supabase.from('user_finances').select('total_spent').eq('user_id', userId).maybeSingle(); return data && data.total_spent >= 100; }, progress: (pet, stats = {}) => Math.min(100, ((stats.total_spent || 0) / 100) * 100) },
    { id: 'task_master', name: 'Task Master', description: 'Complete 5 tasks', icon: '🏆', asyncCheck: async (userId) => { const { count } = await supabase.from('tasks').select('id', { count: 'exact', head: true }).eq('user_id', userId).eq('completed', true); return (count || 0) >= 5; }, progress: (pet, stats = {}) => Math.min(100, ((stats.tasks_done || 0) / 5) * 100) },
    { id: 'vet_visitor', name: 'Veterinarian', description: 'Visit the vet at least once', icon: '🏥', asyncCheck: async (userId, petId) => { const { count } = await supabase.from('expenses').select('id', { count: 'exact', head: true }).eq('user_id', userId).eq('pet_id', petId).eq('expense_type', 'vet'); return (count || 0) >= 1; }, progress: (pet, stats = {}) => Math.min(100, (stats.vet_count || 0) >= 1 ? 100 : 0) },
    { id: 'toy_collector', name: 'Toy Collector', description: 'Buy 3 toys for your pet', icon: '🧸', asyncCheck: async (userId, petId) => { const { count } = await supabase.from('expenses').select('id', { count: 'exact', head: true }).eq('user_id', userId).eq('pet_id', petId).eq('expense_type', 'toy'); return (count || 0) >= 3; }, progress: (pet, stats = {}) => Math.min(100, ((stats.toy_count || 0) / 3) * 100) }
];

async function loadAchievements() {
    const achievementsList = document.querySelector('.achievement-grid');
    const { data: userAchievements } = await supabase.from('achievements').select('*').eq('user_id', currentUser.id).eq('pet_id', petId);
    const completedAchievements = new Set(userAchievements?.map(a => a.achievement_id) || []);
    ...
    } catch (e) { stats.balance = 0; stats.total_spent = 0; }

    try { const tasksRes = await supabase.from('tasks').select('id', { count: 'exact', head: true }).eq('user_id', currentUser.id).eq('completed', true); stats.tasks_done = tasksRes.count || 0; } catch (e) { stats.tasks_done = 0; }
    try { const vetRes = await supabase.from('expenses').select('id', { count: 'exact', head: true }).eq('user_id', currentUser.id).eq('pet_id', petId).eq('expense_type', 'vet'); stats.vet_count = vetRes.count || 0; } catch (e) { stats.vet_count = 0; }
    try { const toyRes = await supabase.from('expenses').select('id', { count: 'exact', head: true }).eq('user_id', currentUser.id).eq('pet_id', petId).eq('expense_type', 'toy'); stats.toy_count = toyRes.count || 0; } catch (e) { stats.toy_count = 0; }
    ...
    achievementsList.innerHTML = ACHIEVEMENTS.map(achievement => {
        ...
        return `<div class="achievement-item ${isCompleted ? 'completed' : ''}"><div class="achievement-icon">${achievement.icon}</div><div class="achievement-info"><div class="achievement-name">${achievement.name}</div><div class="achievement-description">${achievement.description}</div><div class="achievement-progress"><div class="achievement-progress-bar" style="width: ${Math.min(100, progress)}%"></div></div></div><div class="achievement-completed">Complete!</div></div>`;
    }).join('');
}

async function checkAchievements() {
    const { data: userAchievements } = await supabase.from('achievements').select('achievement_id').eq('user_id', currentUser.id).eq('pet_id', petId);
    const completedAchievements = new Set(userAchievements?.map(a => a.achievement_id) || []);
    const { data: finances } = await supabase.from('user_finances').select('*').eq('user_id', currentUser.id).maybeSingle();
    const stats = { balance: finances?.balance || 0, total_spent: finances?.total_spent || 0 };
    ...
    for (const achievement of ACHIEVEMENTS) {
        ...
        if (achievement.asyncCheck) {
            try { achieved = await achievement.asyncCheck(currentUser.id, petId); } catch (err) { achieved = false; console.warn('Achievement asyncCheck failed', achievement.id, err); }
        } else if (achievement.check) {
            achieved = achievement.check(currentPet);
        }
        if (achieved) {
            inserts.push({ user_id: currentUser.id, pet_id: petId, achievement_id: achievement.id, completed_at: new Date().toISOString() });
        }
    }
    ...
    if (inserts.length > 0) {
        ...
        if (insertErr) { console.error('Error inserting achievements', insertErr); } 
    ...
    if (toShow.length > 0) { toShow.forEach(a => shownAchievementIds.add(a.id)); }
...
async function checkAuth() {
    try {
        ...
        if (error || !user) { await delay(300); window.location.href = 'home.html'; return; }
        currentUser = user;
        if (!petId) { await delay(300); window.location.href = 'index.html'; return; }
        await Promise.all([ensureFinance(currentUser.id), loadPet(), loadUserBalance(), loadExpenses(), loadTasks(), loadAchievements(), loadTotalSpent(), loadSavingsGoal(), delay(500)]);
    ...
    } catch (err) { console.error('ERROR in checkAuth:', err); await delay(300); window.location.href = 'home.html'; }
}

async function loadPet() {
    const { data: pet, error } = await supabase.from('pets').select('*').eq('id', petId).eq('owner_id', currentUser.id).maybeSingle();
    if (!pet || error) { alert('Pet not found'); window.location.href = 'index.html'; return; }
    currentPet = pet;
    displayPet();
...
function startStatDecay() {
    setInterval(async () => {
        ...
        if (Math.random() > 0.7) { currentPet.happiness = Math.max(0, currentPet.happiness - 1); }
        if (currentPet.hunger < 15 || currentPet.cleanliness < 15) { currentPet.health = Math.max(0, currentPet.health - 1); }
        await updatePetInDatabase();
        displayPet();
    }, 30000);
}
...
function updateStatBar(stat, value) {
    ...
    if (value < 30) { bar.style.background = 'linear-gradient(90deg, #dc2626, #ef4444)'; }
    else if (value < 60) { bar.style.background = 'linear-gradient(90deg, #f59e0b, #fbbf24)'; }
    else { bar.style.background = 'linear-gradient(90deg, var(--fbla-blue-600), var(--fbla-gold))'; }
}
...
function getPetStatus() {
    ...
    if (currentPet.health < 20) return 'Needs urgent medical attention! 🏥';
    if (currentPet.hunger < 15) return 'Starving and weak... 🍽️';
    if (currentPet.cleanliness < 15) return 'Desperately needs a bath! 🛁';
    if (currentPet.energy < 15) return 'Exhausted and can barely move... 😴';
    const lowStats = [currentPet.hunger < 30, currentPet.happiness < 30, currentPet.cleanliness < 30, currentPet.energy < 30].filter(Boolean).length;
    if (lowStats >= 3) return 'In really bad shape - needs lots of care! 😰';
    if (lowStats >= 2) return 'Struggling with multiple needs... 😟';
    if (avgStat > 90) return 'Living the dream! Absolutely perfect! ✨';
    if (avgStat > 85) return 'Thriving and full of life! 🌟';
    if (avgStat > 75) return 'Very happy and content! 😊';
    if (avgStat > 65) return 'Happy and healthy! 💚';
    if (avgStat > 55) return 'Doing pretty well overall! 👍';
    if (avgStat > 45) return 'Doing okay, but could use some attention 🤔';
    if (avgStat > 35) return 'Starting to feel neglected... 😕';
    if (avgStat > 25) return 'Not doing well, needs care soon! ⚠️';
    if (avgStat > 15) return 'Struggling badly and needs immediate care! 🚨';
    return 'Critical condition! Needs help NOW! 🆘';
}

function getMoodEmoji() {
    const avgStat = (currentPet.hunger + currentPet.happiness + currentPet.energy + currentPet.cleanliness + currentPet.health) / 5;
    if (currentPet.happiness > 90 && avgStat > 80) return '🤩';
    if (currentPet.energy < 20) return '😴';
    if (currentPet.hunger < 20) return '😫';
    if (currentPet.cleanliness < 20) return '🤢';
    if (currentPet.health < 30) return '🤒';
    if (avgStat > 85) return '😄';
    if (avgStat > 70) return '😊';
    if (avgStat > 55) return '🙂';
    if (avgStat > 40) return '😐';
    if (avgStat > 25) return '😟';
    if (avgStat > 15) return '😢';
    return '😭';
}

function getPetEmoji(species) {
    const emojis = { dog: '🐶', cat: '🐱', bird: '🐦', fish: '🐠', mouse: '🐭' };
    return emojis[species] || '🐾';
}

// CONTINUED IN NEXT MESSAGE DUE TO LENGTH...

// CONTINUATION OF pet.js (Part 2)

async function loadUserBalance() {
    const { data, error } = await supabase.from('user_finances').select('balance').eq('user_id', currentUser.id).maybeSingle();
    if (error) { console.error('Error loading user balance:', error); return; }
    if (data) {
...
async function loadExpenses() {
    const { data: expenses, error } = await supabase.from('expenses').select('*').eq('user_id', currentUser.id).eq('pet_id', petId).order('created_at', { ascending: false });
...
function setupEventListeners() {
    ...
    document.getElementById('settings-btn').addEventListener('click', () => {
        document.body.style.transition = 'opacity 0.3s ease-out';
        document.body.style.opacity = '0';
        setTimeout(() => { window.location.href = '../settings/settings.html'; }, 300);
    });
...
async function performAction(action, cost) {
    ...
    const actionMap = {
        feed: () => { currentPet.hunger = Math.min(100, currentPet.hunger + 30); currentPet.happiness = Math.min(100, currentPet.happiness + 5); return { item: 'Pet Food', type: 'food' }; },
        play: () => { currentPet.happiness = Math.min(100, currentPet.happiness + 20); currentPet.energy = Math.max(0, currentPet.energy - 10); currentPet.hunger = Math.max(0, currentPet.hunger - 5); return { item: 'Playtime', type: 'toy' }; },
        clean: () => { currentPet.cleanliness = Math.min(100, currentPet.cleanliness + 40); currentPet.happiness = Math.min(100, currentPet.happiness + 10); return { item: 'Bath & Grooming', type: 'supplies' }; },
        rest: () => { currentPet.energy = Math.min(100, currentPet.energy + 30); currentPet.hunger = Math.max(0, currentPet.hunger - 5); return { item: 'Rest', type: 'rest' }; },
        vet: () => { currentPet.health = 100; currentPet.happiness = Math.max(0, currentPet.happiness - 10); return { item: 'Veterinary Care', type: 'vet' }; },
        toy: () => { currentPet.happiness = Math.min(100, currentPet.happiness + 30); return { item: 'New Toy', type: 'toy' }; }
    };

    const result = actionMap[action]();
    await updatePetInDatabase();
    if (cost > 0 && result) { await deductBalance(cost); await addExpense(result.item, result.type, cost); await loadExpenses(); }
...
async function updatePetInDatabase() {
    ...
    if (error) { console.error('Error saving pet to database:', error); }
    checkAchievements();
}
...
async function addExpense(itemName, type, amount) {
    await supabase.from('expenses').insert({ pet_id: petId, user_id: currentUser.id, expense_type: type, item_name: itemName, amount: amount });
}

async function fetchBalanceHistory() {
    const { data: expenses } = await supabase.from('expenses').select('amount, created_at').eq('user_id', currentUser.id).order('created_at', { ascending: true });
    const { data: tasks } = await supabase.from('tasks').select('reward_amount, completed_at').eq('user_id', currentUser.id).eq('completed', true).order('created_at', { ascending: true });

    const events = [];
    if (expenses) { expenses.forEach(exp => events.push({ type: 'expense', amount: exp.amount, date: new Date(exp.created_at) })); }
    if (tasks) { tasks.forEach(task => events.push({ type: 'task', amount: task.reward_amount, date: new Date(task.completed_at) })); }
    events.sort((a, b) => a.date - b.date);

    let currentBalance = (await loadUserBalance()) || 0;
    const balanceHistory = [{ date: new Date(), balance: currentBalance }];

    for (let i = events.length - 1; i >= 0; i--) {
        const event = events[i];
        if (event.type === 'expense') { currentBalance += event.amount; }
        else if (event.type === 'task') { currentBalance -= event.amount; }
...
function renderSpendingChart(expenses) {
    ...
    const categories = {};
    expenses.forEach(exp => { categories[exp.expense_type] = (categories[exp.expense_type] || 0) + exp.amount; });
    const data = {
        labels: Object.keys(categories),
        datasets: [{ data: Object.values(categories), backgroundColor: ['#FF6384', '#36A2EB', '#FFCE56', '#4BC0C0', '#9966FF', '#FF9F40'], hoverBackgroundColor: ['#FF6384', '#36A2EB', '#FFCE56', '#4BC0C0', '#9966FF', '#FF9F40'] }]
    };
    if (spendingChartInstance) { spendingChartInstance.destroy(); }
    spendingChartInstance = new Chart(ctx, { type: 'pie', data: data, options: { responsive: true, plugins: { legend: { position: 'top', labels: { color: getComputedStyle(document.documentElement).getPropertyValue('--text-secondary') } } } } });
}
...
function renderBalanceHistoryChart(historyData) {
    ...
    const data = { labels: labels, datasets: [{ label: 'Balance', data: dataPoints, fill: false, borderColor: getComputedStyle(document.documentElement).getPropertyValue('--fbla-blue-600'), tension: 0.1 }] };
    if (balanceHistoryChartInstance) { balanceHistoryChartInstance.destroy(); }
    balanceHistoryChartInstance = new Chart(ctx, { type: 'line', data: data, options: { responsive: true, plugins: { legend: { labels: { color: getComputedStyle(document.documentElement).getPropertyValue('--text-muted') } } }, scales: { x: { ticks: { color: getComputedStyle(document.documentElement).getPropertyValue('--text-muted') } }, y: { ticks: { color: getComputedStyle(document.documentElement).getPropertyValue('--text-muted') } } } } });
}

async function loadTasks() {
    const { data: tasks } = await supabase.from('tasks').select('*').eq('user_id', currentUser.id).eq('completed', false).order('created_at', { ascending: false });
    const tasksList = document.getElementById('tasks-list');
    if (!tasks || tasks.length === 0) { tasksList.innerHTML = '<p class="no-data">No tasks available. Generate new ones!</p>'; return; }
    tasksList.innerHTML = '';
    for (const task of tasks) {
        const isIncorrect = incorrectTaskIds.has(task.id);
        const taskItem = document.createElement('div');
        taskItem.className = 'task-item';
        if (isIncorrect) { taskItem.classList.add('incorrect'); }
        ...
        if (isIncorrect) { button.textContent = 'Incorrect'; button.disabled = true; }
...
async function triggerTaskQuestion(taskId) {
    ...
    currentTaskInfo = { id: task.id, reward: Number(task.reward_amount) || 0, question: FBLA_QUESTIONS[Math.floor(Math.random() * FBLA_QUESTIONS.length)] };
...
async function handleSubmitAnswer() {
    ...
    if (isCorrect) {
        ...
        try {
            const { data: currentFinance } = await supabase.from('user_finances').select('*').eq('user_id', currentUser.id).single();
            ...
            const { data: updatedFinance } = await supabase.from('user_finances').update({ balance: newBalance, total_earned: newTotalEarned }).eq('user_id', currentUser.id).select().single();
        ...
        if (taskItem) {
            ...
            const btn = taskItem.querySelector('.task-complete-btn');
            if (btn) { btn.textContent = 'Incorrect'; btn.disabled = true; }
        }
    }
...
async function generateTasks() {
    ...
    try {
        await supabase.from('tasks').delete().eq('user_id', currentUser.id).eq('status', 'pending');
        const inserts = selectedTasks.map(t => ({ user_id: currentUser.id, task_name: t.name, reward_amount: t.reward, completed: false }));
        const { error } = await supabase.from('tasks').insert(inserts);
        if (error) throw error;
        await loadTasks();
    } catch (err) { console.error('Error generating tasks:', err); alert('Unable to generate tasks right now. Check console for details.'); }
}

async function loadTotalSpent() {
    const { data } = await supabase.from('user_finances').select('total_spent').eq('user_id', currentUser.id).maybeSingle();
    if (data) { document.getElementById('total-spent-amount').textContent = `$${data.total_spent.toFixed(2)}`; }
}

async function loadSavingsGoal() {
    const { data: goal } = await supabase.from('savings_goals').select('*').eq('user_id', currentUser.id).eq('pet_id', petId).order('created_at', { ascending: false }).limit(1).maybeSingle();
    if (goal) {
...
async function updateSavingsGoalProgress() {
    ...
    if (balance >= goal.goal_amount && !shownSavingsGoalPopup) { showSavingsGoalPopup(); shownSavingsGoalPopup = true; }
}
...
async function setSavingsGoal() {
    ...
    const { error } = await supabase.from('savings_goals').insert({ user_id: currentUser.id, pet_id: petId, goal_amount: goalAmount });
    if (error) { console.error('Error setting savings goal:', error); alert('Failed to set savings goal.'); return; }
